...
router.post('/', authenticateToken, authorize('create'), (req, res) => {
  const products = readProducts()
  const productIndex = products.findIndex(p => p.name === req.body.name)
  if (productIndex !== -1)
    return res.status(404).json({ message: 'This product exists.' })
...
router.put('/:id', authenticateToken, authorize('update'), (req, res) => {
  const products = readProducts()
...
router.delete('/:id', authenticateToken,authorize('delete'),  (req, res) => {
  const products = readProducts()
...
router.delete('/', authenticateToken, authorize('delete'), (req, res) => {
  const { ids } = req.body
